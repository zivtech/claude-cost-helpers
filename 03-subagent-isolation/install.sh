#!/bin/bash
# Subagent Isolation helper — installer
#
...
HOOK_DIR="${CLAUDE_DIR}/hooks/cost-helpers/subagent-isolation"
COMMANDS_DIR="${CLAUDE_DIR}/commands"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

echo ""
echo "Installing: Subagent Isolation helper"
echo "======================================="
echo ""
...
mkdir -p "$HOOK_DIR"
cp "${SCRIPT_DIR}/file-count-monitor.sh" "${HOOK_DIR}/file-count-monitor.sh"
chmod +x "${HOOK_DIR}/file-count-monitor.sh"
echo "      → ${HOOK_DIR}/file-count-monitor.sh"

# 3. Install slash command (with backup if it already exists)
echo "[2/3] Installing slash commands..."
mkdir -p "$COMMANDS_DIR"
for cmd in delegate; do
...
echo "  2. Read 50+ distinct files (or set CLAUDE_FILE_THRESHOLD=5 temporarily)"
echo "  3. You should see FILE COUNT WARNING in Claude's response context"
echo ""
