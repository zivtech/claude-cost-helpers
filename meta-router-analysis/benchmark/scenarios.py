...
# Each bundle is measured twice: once at its current default-branch state
# ("before") and once at the router-v2 branch that was pushed upstream
# ("after"). Both are public.
REPOS = [
    {
        "label": "drupal-meta-skills (before)",
        "url": "https://github.com/zivtech/drupal-meta-skills.git",
        "ref": "main",
    },
    {
        "label": "drupal-meta-skills (after, router-v2-trim)",
        "url": "https://github.com/zivtech/drupal-meta-skills.git",
        "ref": "router-v2-trim",
    },
    {
        "label": "a11y-meta-skills (before)",
        "url": "https://github.com/zivtech/a11y-meta-skills.git",
        "ref": "main",
    },
    {
        "label": "a11y-meta-skills (after, router-v2-refactor)",
        "url": "https://github.com/zivtech/a11y-meta-skills.git",
        "ref": "router-v2-refactor",
    },
]
...
def ensure_cloned(cache_dir: Path) -> list[tuple[str, Path]]:
    cache_dir.mkdir(parents=True, exist_ok=True)
    out: list[tuple[str, Path]] = []
    for spec in REPOS:
        # Slugify the label for the directory name so before/after sit
        # in separate checkouts.
        slug = spec["label"].replace("(", "").replace(")", "").replace(",", "").replace(" ", "_")
        target = cache_dir / slug
        if target.exists():
            out.append((spec["label"], target))
            continue
        print(f"[clone] {spec['url']} @ {spec['ref']} -> {target}", file=sys.stderr)
        result = subprocess.run(
            ["git", "clone", "--depth", "1", "--branch", spec["ref"], spec["url"], str(target)],
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            print(result.stderr, file=sys.stderr)
            raise SystemExit(f"clone failed for {spec['label']}")
        out.append((spec["label"], target))
    return out
