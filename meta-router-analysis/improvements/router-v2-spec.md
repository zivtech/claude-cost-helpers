...
Baseline numbers come from `../benchmark/results.md`. The benchmark measures both bundles at two refs each — `main` ("before") and the public `router-v2-trim` / `router-v2-refactor` branches ("after") — under three scenarios: **F. Flattened** (straw-man — all SKILL.md and agent bodies preloaded), **A. As-implemented** (at that ref), and **C. Optimized** (simulated). drupal-meta-skills on `main` saved 83% vs the straw-man; `router-v2-trim` brings it to 88% (a 31% reduction in turn-10 parent context). a11y-meta-skills on `main` saved only 30% because its SKILL.md files contained the full protocol inline; `router-v2-refactor` brings it to 95% (a 96.5% reduction in turn-10 parent context). The empirical "after" measurements land within 4% of the simulated C row for the same bundle, validating the simulator for any bundle not yet refactored.
...
**Change.** A `SKILL.md` becomes a stub. Frontmatter carries `name` and `description`; the body holds at most a routing rationale and a `handoff: <agent-subtype>` directive. The real protocol lives in a matching file under `.claude/agents/`. drupal-meta-skills on `main` already does this well (`drupal-planner/SKILL.md` is 2,124 tokens and delegates to `Agent(subagent_type="drupal-planner")`). a11y-meta-skills on `main` did not — the 17,716-token `a11y-planner/SKILL.md` carried the full protocol. The `router-v2-refactor` branch fixes this: the file is now a 258-token stub that delegates to the 9,279-token `a11y-planner.md` agent.

**Empirical saving (a11y, `main` → `router-v2-refactor`).** 41,249 → 558 SKILL.md body tokens (-99%). Combined with leaving stubs in parent: 41,649 → 965 turn-1 parent tokens (-98%).
